...
from singletons import render_html, get_form_data, get_cookie
...
def handle_collections(sam: ServerAccountManager, hashed_token: str) -> Response:
    ...
    if request.method == "GET" and isinstance(target_account, Account):
        return render_html(
            "collections.html",
            sam,
        ...
        if input_type == "create_collection_input":
            ...
            if isinstance(target_account, Account):
                if sam.has_user_collection(collection_name):
                    warn(f'{request.remote_addr}:{get_cookie("account-token")} tried to create already existing collection')
                    flash('Collection already exists', 'warning')
                    if source.startswith("/cheat-sheet/"):
                        return make_response(redirect(source))
                    return render_html(
                        "collections.html",
                        sam,
                        username=target_account.get_username(),
                    ...
                    inform(f'{request.remote_addr}:{get_cookie("account-token")} created collection : {collection_name}, {target_account.id}, {is_public}')
                    flash("Collection created", "success")
                    if source.startswith("/cheat-sheet/"):
                        return make_response(redirect(source))
                return make_response(redirect(f"/collections/{hashed_token}"))
            ...
            inform(f'{request.remote_addr}:{get_cookie("account-token")} deleted collection {collection_name}')
            return make_response(redirect(f"/collections/{hashed_token}"))
            ...
            inform(f'{request.remote_addr}:{get_cookie("account-token")} saved collection {collection_name}')
            return make_response(redirect(f"/cheat-sheet/{cheat_sheet_token}"))
            ...
            inform(f'{request.remote_addr}:{get_cookie("account-token")} removed cheat-sheet: {cheat_sheet_token} from collection: {collection_name}')
            return make_response(redirect(f"/cheat-sheet/{cheat_sheet_token}"))
        

    return make_response("WIP, come back later! ^^")
...
def handle_collection(
    ...
    author: Account | None = sam.get_account_from_hashed_token(hashed_token)
    if author is None:
        return make_response(abort(404))
    is_user: bool = author.get_id() == sam.get_user_account_token()
    cheat_sheet_tokens: list | None = sam.get_cheat_sheet_token_from_collection(author.get_id(), collection_name)
    cheat_sheet_info = []
    ...
    if request.method == "POST":
        ...
        if input_type == "rename_collection_input":
            ...
            inform(f'{request.remote_addr}:{get_cookie("account-token")} renamed collection {collection_name}, to {new_collection_name}')
            flash('collection renamed','success')
            return make_response(redirect(f"/collections/{hashed_token}/{new_collection_name}"))
            ...
            inform(f'{request.remote_addr}:{get_cookie("account-token")} modified the visibility of {collection_name}')
        ...
        return make_response(redirect(f"/collections/{hashed_token}/{collection_name}"))
